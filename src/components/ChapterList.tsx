...
export function ChapterList() {
  ...
  return (
    <div className="space-y-6">
      ...
      {ready && (
        <Reveal>
          <div className="shine-host rounded-3xl bg-gradient-to-br from-amber-400 to-orange-500 text-white p-6 sm:p-8 shadow-lg shadow-amber-200/60">
            {started && resume ? (
              <>
                <p className="font-bold text-amber-50 mb-2">
                  지금까지 <span className="text-white text-xl font-extrabold">{percent}%</span> 왔어요. 잘하고
                  있어요! 👏
                </p>
                <div className="w-full h-3.5 rounded-full bg-white/25 mb-5 overflow-hidden">
                  <div
                ...
                <Link
                  ...
                  className="block text-center rounded-2xl bg-white text-orange-600 font-extrabold text-xl px-6 py-4 btn-press shadow-md"
                >
                  하던 곳부터 이어하기 →
                </Link>
              </>
              ...
              <>
                <p className="font-bold text-amber-50 mb-4 text-center sm:text-left">
              ...
              <>
                ...
                <p className="text-amber-50">필요할 때 언제든 다시 찾아보세요.</p>
              </>
            )}
          </div>
        </Reveal>
          ...
          return (
            <li key={c.slug}>
              <Reveal delay={Math.min(i * 70, 350)}>
                <Link
                  href={`/guide/${c.slug}`}
                  className="group flex items-center gap-4 rounded-2xl bg-white border border-stone-200/80 hover:border-amber-300 hover:shadow-md hover:-translate-y-0.5 p-4 sm:p-5 transition-all duration-300"
                >
                  <span
                  ...
                  <span className="shrink-0 text-right">
                    ...
                    <span className="block text-amber-400 opacity-0 -translate-x-1 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300 mt-0.5">
                      →
                    </span>
                  </span>
                </Link>
